...
User keybag 是设备常规操作中使用的封装类密钥的储存位置（与用户 passcode 相关），设备密钥包用来储存用于涉及设备特定操作数据的封装类密钥（仅与 UID 相关），两者合称为系统密钥包。
...
- CLAS[2] 有个特殊的字段`PBKY`，保存了 Class B 类密钥的**私钥**
...
- Module-managed keybags key：= `Bag1`
    用于用户密钥包内容的加密/解密，基于`Key 0x835`以 AES-256 KW 封装，密文持久化存储？// TODO:
