...
小程序的运行环境分成**渲染层**和**逻辑层**，分别由2个线程管理：
...
- 正是由于js 的运行进程与渲染进程(视图层)是独立的，因此视图的渲染更新，并不会阻塞 js 的执行，js 的逻辑执行，也不会阻塞视图的渲染更新。

- 由于JSCore 的环境同 NodeJS 环境不尽相同，所以一些 NPM 的包在小程序中也是无法运行的。
