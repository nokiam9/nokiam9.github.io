...
需要注意的是，在 web 开发中，我们每个页面中 js 线程与渲染线程是互斥的，这也是为什么长时间的脚本运行可能会导致页面失去响应，而小程序的 js，是运行在一个称为“逻辑层”的独立运行环境中（大多数情况下），每个小程序有且仅有一个“逻辑层”，js 的运行进程与渲染进程(视图层)是独立的，视图的渲染更新，并不会阻塞 js 的执行，js 的逻辑执行，也不会阻塞视图的渲染更新。

下图是借用头条的逻辑结构图，概念是相同的。

{% asset_img runtime-model.png %}

需要注意的问题：js 驱动视图的更新是异步的，而且 js 无法直接访问视图的 DOM。

由于小程序的逻辑层和渲染层是分开的，**逻辑层运行在 JSCore 中，并没有一个完整浏览器对象，因而缺少相关的DOM API和BOM API**。这一区别导致了前端开发非常熟悉的一些库，例如 jQuery、 Zepto 等，在小程序中是无法运行的。
