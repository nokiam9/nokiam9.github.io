...
对于读卡器模式，NFC Forum定义了4种标准类型的Tag标签（Type 5还不是正式标准），这四种类型NFC Tag的区别在于存储空间大小，数据传输率以及底层使用的协议上，后面有具体的技术细节。
...
该模式下，NFC 手机的工作类似于标准的非接触式智能卡，为此ISO 14443定义了Type A 和 Type B两种技术标准（Type F不是正式标准），后续有详细技术分析。
