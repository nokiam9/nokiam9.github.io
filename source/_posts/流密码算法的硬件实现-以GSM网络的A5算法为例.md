...
A5/1 算法曾经是使用最广泛的GSM加密算法，在设备中的支持程度也是几种算法中最高的。该算法在1994年被初步泄露，在1999年通过逆向工程的方式被公布出来，从2000年开始即被逐步破解。
...
由于受巴统限制，A5/1 算法作为出口管制技术无法集成到在中国境内使用的设备中，为此推出了弱化版本的 A5/2 算法，但一经公布即被发现存在缺陷（后门）可以被实时破解，随后 3GPP标准 TS 33.020 中已明确禁止使用。
