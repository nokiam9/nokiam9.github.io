...
1993年，Shimon Even 和 Yishay Mansour 研究了[DES-X 密钥白化机制](https://link.springer.com/content/pdf/10.1007/s001459900025)。
...
1. tweak key 实际上是一种流密码，解密和加密的算法完全相同；但是用于 AES 的主密钥的加密和解密算法是互逆的。
2. 基于 key2 和 i 完成 T 的初始化，后续每个明文分组持续更新 T。算子实际是基于GF(2^128)有限域的 j 阶乘法运算，可以用一个 LSFR（线性移位反馈寄存器）完成硬件实现。
3. tweak key 基于 XOR-ENCRYPT-XOR 模式，更改密钥时不需要掌握原密钥，也不需要更新已存储的密文数据，这个特点对于磁盘级数据加密非常有价值！
